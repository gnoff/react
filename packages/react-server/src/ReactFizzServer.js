...
function createSuspenseBoundary(
  ...
  return {
    ...
    errorDigest: null,
    contentState: createBoundaryState(),
    fallbackState: createBoundaryState(),
...
function createRenderTask(
  ...
  childIndex: number,
  blockedBoundary: Root | SuspenseBoundary,
  blockedBoundaryState: null | BoundaryState,
...
function renderSuspenseBoundary(
  ...
  const parentBoundaryState = task.blockedBoundaryState;
  ...
  task.blockedBoundaryState = newBoundary.contentState;
  ...
  try {
    ...
    task.blockedBoundaryState = parentBoundaryState;
...
function replaySuspenseBoundary(
  ...
  task.blockedBoundaryState = resumedBoundary.contentState;
  ...
  switch (segment.status) {
    ...
    case COMPLETED: {
      ...
      for (let childIdx = 0; childIdx < children.length; childIdx++) {
        ...
        r = flushSegment(request, destination, nextChild, boundaryState);
      }
...
function flushSegment(
  ...
  if (boundary === null) {
    // Not a suspense boundary.
    return flushSubtree(request, destination, segment, boundaryState);
  }
  ...
  if (boundary.status === CLIENT_RENDERED) {
    ...
    // Flush the fallback.
    flushSubtree(request, destination, segment, boundaryState);
    ...
    if (enableFloat) {
      if (boundaryState) {
        hoistToBoundary(boundaryState, boundary.fallbackState);
      }
    }
    // Flush the fallback.
    flushSubtree(request, destination, segment, boundaryState);
    ...
    if (enableFloat) {
      if (boundaryState) {
        hoistToBoundary(boundaryState, boundary.contentState);
      }
    }
    ...
    const contentSegment = completedSegments[0];
    flushSegment(request, destination, contentSegment, boundaryState);
...
function flushSegmentContainer(
  ...
  flushSegment(request, destination, segment, boundaryState);
...
function flushPartiallyCompletedSegment(
  ...
  const boundaryState = boundary.contentState;
  ...
  if (segmentID === -1) {
    ...
    return flushSegmentContainer(request, destination, segment, boundaryState);
    ...
    flushSegmentContainer(request, destination, segment, boundaryState);
